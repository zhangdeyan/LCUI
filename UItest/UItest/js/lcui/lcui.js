var UI = {
    ...
    "start": function (cb) {
        ...
        for (var i = 0; i < UI.layerNum; i++) {
            ...
            UI.rootWin[i] = new UIFrame(p);
            UI.focusWin[i] = UI.rootWin[i];
        }
...
function UIModule() {
    ...
    this.defOpen = function () {
        var self = this;
        if (!this.win) {
            this.win = UI.createGroup(this.dlgParam, UI.rootWin[0], null);
...
function UIWm() {
    ...
    this.setFocus = function (value) {
        ...
        if (value && this.focusStop) {
            ...
            if (UI.focusWin[rootId] != this) {

                if (UI.focusWin[rootId]) {
                    UI.focusWin[rootId].setFocus(false);
                }
                UI.focusWin[rootId] = this;
            }
    ...
    this.releaseFocus = function () {
        ...
        if (UI.focusWin[rootId] == this) {
            ...
            ret = this.brotherGetFocus({keyCode: UI.KEY.LEFT});
    ...
    this.brotherGetFocus = function (e) {
        ...
        for (var i = 0; i < arr.length; i++) {
            if (arr[i].id == this.nextFocusLeft && e.keyCode == UI.KEY.LEFT) {
                ret = true;
            }
            else if (arr[i].id == this.nextFocusRight && e.keyCode == UI.KEY.RIGHT) {
                ret = true;
            }
            else if (arr[i].id == this.nextFocusUp && e.keyCode == UI.KEY.UP) {
                ret = true;
            }
            else if (arr[i].id == this.nextFocusDown && e.keyCode == UI.KEY.DOWN) {
                ret = true;
            }
        ...
        if (!ret) {
            ...
            else if (e.keyCode == UI.KEY.RIGHT) {
                d = 3;
            }
...
/**
 *  getJqElement,getJsElement,getMarqueeValue
 * @constructor
 */
function UIMarquee(params) {
    ...
    this.initMarquee = function (params) {
        this.params = params
    };
    ...
    this.Moveleft = function () {
        if (self.MarqueeTimer) {
            ...
            if (this.marqueeInfo.value == ('' + this.params.getMarqueeValue())) {
                this.params.getJqElement().text(this.marqueeInfo.temp);
            }
...
function UIFrame(attr) {
    ...
    this.create = function () {
        ...
        if (this.attr && typeof(this.attr) == 'object') {
            this.setCssAttr(this.attr, this.element);
        }
...
function UISelect(attr) {
    ...
    this.defProc = function (e) {
        ...
        if (ret == true) {
            UI.onKey({keyCode: UI.KEY.SELECT_VALUE_CHANGE, id: this.id, hwin: this});
        }
        return ret;
    };
    this.create();
...
UISelect.prototype = new UIWm();


function UIEdit(attr) {
    this.password = false;
    this.maxChars = 4;
    this.value = "";
    this.suffixValue = "";
    ...
    this.defProc = function (e) {
        ...
        if (ret == true) {
            this.setValue();
            UI.onKey({keyCode: UI.KEY.EDIT_VALUE_CHANGE, id: this.id, hwin: this});
        }
        return ret;
    };
...
function UIScrollBar(attr) {
    ...
    this.render = function () {
        ...
        if (this.scrollType == "V") {
            ...
            var t = (this.curIndex / this.totalNum) * (parseInt(this.element.css('height')) - h);
            ...
            this.element.children('div').css('position', 'relative');
            this.element.children('div').css('width', w + "px");
            var l = (this.curIndex / this.totalNum) * (parseInt(this.element.css('width')) - w);
...
function UITable(attr) {
    ...
    this.listInit(this.rows, this.rowsOnePage, this.curIndex);
    ...
    this.defProc = function (e) {
        ...
        else if (e.keyCode == UI.KEY.ENTER) {
